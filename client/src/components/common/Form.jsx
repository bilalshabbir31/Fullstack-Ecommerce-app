...
const CommonForm = ({ formControls, formData, setFormData, onSubmit, buttonText }) => {
  ...
  function renderInputsByComponentType(getControlItem) {
    ...
    switch (getControlItem.componentType) {
      ...
      default:
        element = (<Input name={getControlItem.name} placeholder={getControlItem.placeholder} id={getControlItem.name} type={getControlItem.type}
          onChange={event => setFormData({
            ...formData,
            [getControlItem.name]: event.target.value
          })} />)
  ...
  return (
    <form onSubmit={onSubmit}>
      <div className="flex flex-col gap-3">
        {
          formControls.map(controlItems => <div key={controlItems.name} className="grid w-full gap-1.5">
